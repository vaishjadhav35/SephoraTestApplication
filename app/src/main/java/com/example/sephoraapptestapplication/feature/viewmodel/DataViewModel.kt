...
class DataViewModel @Inject constructor(private val repository: DataRepository) :
    ...
    fun observerApiResponse() {
        _getResponse.observeForever(Observer { response ->
            if (response.statusCode != 200) {
                var errorResponseMsg =
                    if (response.error != null) response.error else response.message
                sendResponseToView(
                ...
                when (response.serviceID) {
                    REPOSITORIES_SERVICE_ID -> {
                        sendResponseToView(
                            ...
                            result = response.result as FetchRepositoryResponse
                        )
                    }
                }
            }
    ...
    private fun sendResponseToView(
        ...
        serviceID: String,
        errorCode: Int = 0,
        result: FetchRepositoryResponse? = null
    ) {
        val responseData =
            FetchRepositoryApiResponse(isSuccess, responseString, errorCode, serviceID, result)
